                                    ...
                                    ACVP_CIPHER alg_id)
{
    ...
    //
    // stc->msg = calloc(1, ACVP_RSA_MSG_MAX);
    // if (!stc->msg) return ACVP_MALLOC_FAIL;
    // stc->mac = calloc(1, ACVP_RSA_MAC_MAX);
    // if (!stc->mac) return ACVP_MALLOC_FAIL;
    // stc->key = calloc(1, ACVP_RSA_KEY_MAX);
    // if (!stc->key) return ACVP_MALLOC_FAIL;
    //
    // rv = acvp_hexstr_to_bin((const unsigned char *)msg, stc->msg, ACVP_RSA_MSG_MAX);
    // if (rv != ACVP_SUCCESS) {
    //     ACVP_LOG_ERR("Hex converstion failure (msg)");
    //     return rv;
    // }
    // rv = acvp_hexstr_to_bin((const unsigned char *)key, stc->key, ACVP_RSA_KEY_MAX);
    // if (rv != ACVP_SUCCESS) {
...
ACVP_RESULT acvp_rsa_kat_handler(ACVP_CTX *ctx, JSON_Object *obj)
{
    ...
    //          */
    //         if (msglen == 0) {
    //             msglen = strnlen((const char *)msg, ACVP_RSA_MSG_MAX) / 2;
    //         }
