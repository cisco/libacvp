                                    ...
                                    unsigned char *q_rand)
{
    ...
    case ACVP_RSA_MODE_KEYGEN:
        ...
        if(info_gen_by_server) {
            if(!seed || !seedlen || !bitlen1 || !bitlen2 || !bitlen3 || !bitlen4) {
                ACVP_LOG_ERR("Missing args for tc generated by server\n");
                return ACVP_MALFORMED_JSON; // not sure if this is the right err code...
            }
            ...
            stc->keygen_tc->seed_len = seedlen;
            stc->keygen_tc->e = &e;
...
static ACVP_RESULT acvp_rsa_output_tc(ACVP_CTX *ctx, ACVP_RSA_TC *stc, JSON_Object *tc_rsp)
{
    ...
    switch(stc->mode) {
        case ACVP_RSA_MODE_KEYGEN:
            tmp = calloc(1, ACVP_RSA_SEEDLEN_MAX);
            if (!tmp) {
                ACVP_LOG_ERR("Unable to malloc in acvp_rsa_output_tc");
                return ACVP_MALLOC_FAIL;
            }
            rv = acvp_bin_to_hexstr(stc->keygen_tc->seed, stc->keygen_tc->seed_len, (unsigned char*)tmp);
            if (rv != ACVP_SUCCESS) {
                ACVP_LOG_ERR("hex conversion failure (rsa seed)");
                return rv;
            }
            json_object_set_string(tc_rsp, "seed", tmp);
    ...
    json_object_set_string(tc_rsp, "primeResult", (const char *)stc->keygen_tc->prime_result);
    json_object_set_string(tc_rsp, "pRand", (const char *)stc->keygen_tc->p_rand);
    json_object_set_string(tc_rsp, "qRand", (const char *)stc->keygen_tc->q_rand);
...
ACVP_RESULT acvp_rsa_kat_handler(ACVP_CTX *ctx, JSON_Object *obj)
{
    ...
    int info_gen_by_server;// TODO unused... , rand_pq;
    // TODO unused... char *hash_alg, *prime_test, *pub_exp;

    /*
     * keygen attrs
     */
    unsigned char *p_rand, *q_rand, *seed;
    unsigned int bitlen1, bitlen2, bitlen3, bitlen4, mod, seedlen;
    BIGNUM *e;
    const char *exponent;
    ...
    for (i = 0; i < g_cnt; i++) {
        ...
        for (j = 0; j < t_cnt; j++) {
            ...
            switch(mode_id) {
                case ACVP_RSA_MODE_KEYGEN:
                    if (info_gen_by_server) {
                        ...
                        if(!seed) return ACVP_INVALID_ARG; // not sure sure this is the right error here...
                        seedlen = strnlen((char *)seed, ACVP_RSA_SEEDLEN_MAX) / 2;
                    }
                    break;
                default:
                    break;
            ...
            acvp_rsa_init_tc(ctx, &stc, tc_id, alg_id, info_gen_by_server,
                             // rand_pq, mod, hash_alg, prime_test, pub_exp,
                             mod,
                            /* keygen params... */
                             seed, seedlen, *e, bitlen1, bitlen2, bitlen3,
                             bitlen4, p_rand, q_rand);
