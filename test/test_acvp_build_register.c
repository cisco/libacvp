...
//TEST_GROUP(BUILD_DEPS);
//TEST_GROUP(BUILD_MODULES);
//TEST_GROUP(BUILD_OES);
TEST_GROUP(BUILD_TEST_SESSION);

static ACVP_CTX *ctx = NULL;
static ACVP_RESULT rv = 0;
static char *cvalue = "same";
static char *reg = NULL;
static JSON_Value *reg_value = NULL;
static JSON_Value *generated_value = NULL;
static JSON_Value *known_good_value = NULL;
static JSON_Object *generated_obj = NULL;
static JSON_Object *known_good_obj = NULL;
...
static void build_vendors_tear_down_helper(void) {
    if (ctx) {
...
TEST_SETUP(BUILD_TEST_SESSION) {}
TEST_TEAR_DOWN(BUILD_TEST_SESSION) {
    build_vendors_tear_down_helper();
}

TEST_SETUP(BUILD_OES) {}
TEST_TEAR_DOWN(BUILD_OES) {
    build_vendors_tear_down_helper();
}

TEST_SETUP(BUILD_VENDORS) {}
TEST_TEAR_DOWN(BUILD_VENDORS) {
    build_vendors_tear_down_helper();
}

TEST_SETUP(BUILD_MODULES) {} 
TEST_TEAR_DOWN(BUILD_MODULES) {
    build_vendors_tear_down_helper();
}

TEST_SETUP(BUILD_DEPS) {}
TEST_TEAR_DOWN(BUILD_DEPS) {
    build_vendors_tear_down_helper();
}
 ...
 */
TEST(BUILD_VENDORS, good_vendors_output) {
    ...
    TEST_ASSERT_EQUAL(JSONSuccess, json_value_equals((JSON_Value *)known_good_obj, (JSON_Value *)generated_obj));
}
 ...
 * the correct json structure
 */
TEST(BUILD_MODULES, good_modules_output) {
 ...
 * the correct json structure
 */
TEST(BUILD_OES, good_oes_output) {
 ...
 * the correct json structure
 */
TEST(BUILD_DEPS, good_deps_output) {
...
TEST(BUILD_TEST_SESSION, np_caps_ctx) {
 ...
 * the correct json structure
 */
TEST(BUILD_TEST_SESSION, good_aes_output) {
    // TODO: Move setup_empty_ctx to TEST_SETUP and remove teardown_ctx from test
    if (ctx) teardown_ctx(&ctx);
    ctx = NULL;
    setup_empty_ctx(&ctx);
    ...
    TEST_ASSERT_EQUAL(JSONSuccess, json_value_equals((JSON_Value *)generated_obj, (JSON_Value *)known_good_obj));
}
 ...
 * (a required val)
 */
TEST(BUILD_TEST_SESSION, missing_required_keylen_aes) {
    // TODO: Move setup_empty_ctx to TEST_SETUP and remove teardown_ctx from test
    if (ctx) teardown_ctx(&ctx);
    ctx = NULL;
    setup_empty_ctx(&ctx);
 ...
 * never called
 */
TEST(BUILD_TEST_SESSION, missing_required_direction_aes) {
    // TODO: Move setup_empty_ctx to TEST_SETUP and remove teardown_ctx from test
    if (ctx) teardown_ctx(&ctx);
    ctx = NULL;
    setup_empty_ctx(&ctx);
...
TEST(BUILD_TEST_SESSION, good_hash) {
    // TODO: Move setup_empty_ctx to TEST_SETUP and remove teardown_ctx from test
    if (ctx) teardown_ctx(&ctx);
    ctx = NULL;
    setup_empty_ctx(&ctx);
...
TEST(BUILD_TEST_SESSION, good_drbg) {
    // TODO: Move setup_empty_ctx to TEST_SETUP and remove teardown_ctx from test
    if (ctx) teardown_ctx(&ctx);
    ctx = NULL;
    setup_empty_ctx(&ctx);
...
TEST(BUILD_TEST_SESSION, drbg_missing_cap_parms) {
    // TODO: Move setup_empty_ctx to TEST_SETUP and remove teardown_ctx from test
    if (ctx) teardown_ctx(&ctx);
    ctx = NULL;
    setup_empty_ctx(&ctx);
 ...
 * the correct json structure
 */
TEST(BUILD_TEST_SESSION, good_cmac_output) {
    // TODO: Move setup_empty_ctx to TEST_SETUP and remove teardown_ctx from test
    if (ctx) teardown_ctx(&ctx);
    ctx = NULL;
    setup_empty_ctx(&ctx);
...
TEST(BUILD_TEST_SESSION, cmac_missing_direction) {
    // TODO: Move setup_empty_ctx to TEST_SETUP and remove teardown_ctx from test
    if (ctx) teardown_ctx(&ctx);
    ctx = NULL;
    setup_empty_ctx(&ctx);
...
// cmac direction attribute never enabled
TEST(BUILD_TEST_SESSION, cmac_missing_tdes_ko) {
    // TODO: Move setup_empty_ctx to TEST_SETUP and remove teardown_ctx from test
    if (ctx) teardown_ctx(&ctx);
    ctx = NULL;
    setup_empty_ctx(&ctx);
...
TEST(BUILD_TEST_SESSION, good_hmac) {
    // TODO: Move setup_empty_ctx to TEST_SETUP and remove teardown_ctx from test
    if (ctx) teardown_ctx(&ctx);
    ctx = NULL;
    setup_empty_ctx(&ctx);
...
TEST(BUILD_TEST_SESSION, good_dsa) {
    // TODO: Move setup_empty_ctx to TEST_SETUP and remove teardown_ctx from test
    if (ctx) teardown_ctx(&ctx);
    ctx = NULL;
    setup_empty_ctx(&ctx);
...
TEST(BUILD_TEST_SESSION, dsa_missing_pqgen) {
    // TODO: Move setup_empty_ctx to TEST_SETUP and remove teardown_ctx from test
    if (ctx) teardown_ctx(&ctx);
    ctx = NULL;
    setup_empty_ctx(&ctx);
...
// dsa registration with missing args
TEST(BUILD_TEST_SESSION, dsa_missing_ggen) {
    // TODO: Move setup_empty_ctx to TEST_SETUP and remove teardown_ctx from test
    if (ctx) teardown_ctx(&ctx);
    ctx = NULL;
    setup_empty_ctx(&ctx);
...
// dsa registration with missing args
TEST(BUILD_TEST_SESSION, dsa_missing_hashalgs) {
    // TODO: Move setup_empty_ctx to TEST_SETUP and remove teardown_ctx from test
    if (ctx) teardown_ctx(&ctx);
    ctx = NULL;
    setup_empty_ctx(&ctx);
 ...
 * the correct json structure
 */
TEST(BUILD_TEST_SESSION, good_des_output) {
    // TODO: Move setup_empty_ctx to TEST_SETUP and remove teardown_ctx from test
    if (ctx) teardown_ctx(&ctx);
    ctx = NULL;
    setup_empty_ctx(&ctx);
...
TEST(BUILD_TEST_SESSION, good_rsa) {
    // TODO: Move setup_empty_ctx to TEST_SETUP and remove teardown_ctx from test
    if (ctx) teardown_ctx(&ctx);
    ctx = NULL;
    setup_empty_ctx(&ctx);
...
TEST(BUILD_TEST_SESSION, rsa_no_params) {
    // TODO: Move setup_empty_ctx to TEST_SETUP and remove teardown_ctx from test
    if (ctx) teardown_ctx(&ctx);
    ctx = NULL;
    setup_empty_ctx(&ctx);
...
TEST(BUILD_TEST_SESSION, good_ecdsa) {
    // TODO: Move setup_empty_ctx to TEST_SETUP and remove teardown_ctx from test
    if (ctx) teardown_ctx(&ctx);
    ctx = NULL;
    setup_empty_ctx(&ctx);
...
TEST(BUILD_TEST_SESSION, ecdsa_no_params) {
    // TODO: Move setup_empty_ctx to TEST_SETUP and remove teardown_ctx from test
    if (ctx) teardown_ctx(&ctx);
    ctx = NULL;
    setup_empty_ctx(&ctx);
...
TEST(BUILD_TEST_SESSION, good_kdf) {
    // TODO: Move setup_empty_ctx to TEST_SETUP and remove teardown_ctx from test
    if (ctx) teardown_ctx(&ctx);
    ctx = NULL;
    setup_empty_ctx(&ctx);
...
TEST(BUILD_TEST_SESSION, kdf_more_modes) {
    // TODO: Move setup_empty_ctx to TEST_SETUP and remove teardown_ctx from test
    if (ctx) teardown_ctx(&ctx);
    ctx = NULL;
    setup_empty_ctx(&ctx);
...
TEST(BUILD_TEST_SESSION, good_kas_ecc) {
    // TODO: Move setup_empty_ctx to TEST_SETUP and remove teardown_ctx from test
    if (ctx) teardown_ctx(&ctx);
    ctx = NULL;
    setup_empty_ctx(&ctx);
...
TEST(BUILD_TEST_SESSION, kas_ecc_no_params) {
    // TODO: Move setup_empty_ctx to TEST_SETUP and remove teardown_ctx from test
    if (ctx) teardown_ctx(&ctx);
    ctx = NULL;
    setup_empty_ctx(&ctx);
...
TEST(BUILD_TEST_SESSION, good_kas_ffc) {
    // TODO: Move setup_empty_ctx to TEST_SETUP and remove teardown_ctx from test
    if (ctx) teardown_ctx(&ctx);
    ctx = NULL;
    setup_empty_ctx(&ctx);
...
// The kas registration never calls params API
TEST(BUILD_TEST_SESSION, kas_ffc_no_params) {
    // TODO: Move setup_empty_ctx to TEST_SETUP and remove teardown_ctx from test
    if (ctx) teardown_ctx(&ctx);
    ctx = NULL;
    setup_empty_ctx(&ctx);
